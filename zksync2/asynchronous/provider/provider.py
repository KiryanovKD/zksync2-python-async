...
from zksync2.asynchronous.manage_contracts.erc20_contract import AsyncERC20Contract
from zksync2.asynchronous.manage_contracts.l1_bridge import AsyncL1Bridge
from zksync2.asynchronous.manage_contracts.l2_bridge import AsyncL2Bridge
from zksync2.asynchronous.manage_contracts.zksync_contract import AsyncZkSyncContract
from zksync2.shared.core.utils import RecommendedGasLimit, to_bytes, is_eth
from zksync2.shared.core.types import Token, BridgeAddresses, EthBlockParams, ZksMessageProof
...
class AsyncEthereumProvider:
    ...
    async def get_l1_balance(self, token: Token, block: EthBlockParams):
        if token.is_eth():
            return await self._eth_web3.eth.get_balance(self.address, block.value)
        else:
            token_contract = AsyncERC20Contract(self._eth_web3.eth,
                                                token.l1_address,
                                                self._l1_account)
        ...
        if token.is_eth():
            ...
            if not self._l1_bridge:
                ...
                self._l1_bridge = AsyncL1Bridge(bridge_addresses.erc20_l1_default_bridge,
                                                self._eth_web3, self._l1_account)
            return await self.l1_bridge.l2_token_address(token.l1_address)
    ...
    async def get_base_cost(self,
        ...
        if gas_price is None:
            gas_price = await self._eth_web3.eth.gas_price
        return await self.main_contract.l2_tx_base_cost(gas_price, gas_limit, gas_per_pubdata_byte)
    ...
    async def approve_erc20(self,
        ...
        erc20 = AsyncERC20Contract(self._eth_web3.eth, token.l1_address, self._l1_account)
    ...
    async def request_execute(self,
        ...
        tx_receipt = await self.main_contract.request_l2_transaction(contract_l2=contract_address,
                                                                     l2_value=l2_value,
                                                                     call_data=call_data,
                                                                     l2_gas_limit=l2_gas_limit,
    ...
    async def finalize_withdrawal(self, withdraw_hash, index: int = 0):
        ...
        if is_eth(params["sender"]):
            return await self.main_contract.finalize_eth_withdrawal(
    ...
    async def is_withdrawal_finalized(self, withdraw_hash, index: int = 0):
        ...
        if is_eth(sender):
            return await self.main_contract.is_eth_withdrawal_finalized(
