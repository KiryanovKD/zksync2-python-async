...
class BaseContractEncoder:
    ...
    @classmethod
    def from_json(cls, web3: Union[Web3, AsyncWeb3], compiled_contract: Path):
        with compiled_contract.open(mode='r') as json_f:
            data = json.load(json_f)
            return cls(web3, abi=data["abi"])

    def __init__(self, web3: Union[Web3, AsyncWeb3], abi, bytecode: Optional[bytes] = None):
        self.web3 = web3
        self.abi = abi
        if bytecode is None:
...
class ContractEncoder(BaseContractEncoder):
    ...
    def __init__(self, web3: Union[Web3, AsyncWeb3], abi, bytecode):
